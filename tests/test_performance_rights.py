...
class TestPerformanceRights(unittest.TestCase):
    def test_not_a_performance_green(self):
        data = base_data()
        data["performance_info"].update({"is_performance": "not_performance"})
        results = run_perf(data)
        assert any(
            r["condition"] == "PublicDomainNotAPerformance" for r in results["green"]
        )
    ...
    def test_before_1900_green(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PublicDomainRuleOfThumbPerformance"
    ...
    def test_compound_info(self):
        data = base_data()
        data["performance_info"].update(
            {"is_performance": "performance", "is_compound_performance": "compound"}
        )
        results = run_perf(data)
        assert any(r["condition"] == "CompoundPerformance" for r in results["info"])
    ...
    def test_unknown_performance_year_yellow(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PerformanceYearUnknown" for r in results["yellow"]
        )
    ...
    def test_eea_never_made_publicly_available_green(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PerformanceProtectionLapsedArticle3S1"
            for r in results["green"]
        )
    ...
    def test_eea_missing_event_year_with_yes_yellow(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PerformanceUnknownPublicationExceptions"
    ...
    def test_noneea_would_be_green_becomes_green(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PerformanceLapsedEvenIfEEA" for r in results["green"]
        )
    ...
    def test_noneea_would_be_red_becomes_yellow(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PerformanceNonEEAUncertain" for r in results["yellow"]
        )
    ...
    def test_cc_license_upgrade_green(self):
        ...
        results = run_perf(data)
        assert any(
            r["condition"] == "PerformanceAvailableCCLicense"
            for r in results["rights_green"]
        )
    ...
    def test_rights_acquisition_upgrade_yellow(self):
        ...
        assert any(
            r["condition"] == "PerformanceOnlineAvailable"
            for r in results["rights_yellow"]
        )
