...
def run_broadcast(data: dict) -> dict:
...
class TestBroadcastingRights(unittest.TestCase):
    ...
    def test_not_a_broadcast_green(self):
        ...
        data = base_data()
        data["broadcast_info"].update({"is_broadcast": "not_broadcast"})
        status = run_broadcast(data)
        assert any(
            r["condition"] == "PublicDomainNotABroadcast" for r in status["green"]
        )
    ...
    def test_broadcast_before_1970_green(self):
        ...
        status = run_broadcast(data)
        assert any(
            r["condition"] == "PublicDomainRuleOfThumbBroadcasts"
    ...
    def test_compound_broadcast_info(self):
        ...
        data = base_data()
        data["broadcast_info"].update(
            {"is_broadcast": "broadcast", "is_compound_broadcast": "compound"}
        )
        status = run_broadcast(data)
    ...
    def test_uncertain_compound_broadcast_info(self):
        ...
        data = base_data()
        data["broadcast_info"].update(
            {"is_broadcast": "broadcast", "is_compound_broadcast": "uncertain"}
        )
        status = run_broadcast(data)
        assert any(r["condition"] == "CompoundBroadcast" for r in status["info"])
    ...
    def test_eea_broadcast_50_years_red(self):
        ...
        status = run_broadcast(data)
        assert any(
            r["condition"] == "BroadcastStillProtectedArticle3" for r in status["red"]
        )
    ...
    def test_eea_broadcast_51_years_green(self):
        ...
        status = run_broadcast(data)
        assert any(
            r["condition"] == "BroadcastProtectionLapsedArticle3"
            for r in status["green"]
        )
    ...
    def test_non_eea_broadcast_50_years_yellow(self):
        ...
        status = run_broadcast(data)
        assert any(
            r["condition"] == "BroadcastNonEEAUncertain" for r in status["yellow"]
        )
    ...
    def test_non_eea_broadcast_51_years_green(self):
        ...
        status = run_broadcast(data)
        assert any(
            r["condition"] == "BroadcastLapsedEvenIfEEA" for r in status["green"]
        )
    ...
    def test_broadcast_rights_acquisition_upgrade_green(self):
        ...
        status = run_broadcast(data)
        assert any(
            r["condition"] == "BroadcastOnlineAvailable" for r in status["rights_green"]
        )
    ...
    def test_broadcast_year_1970_boundary(self):
        ...
        if current - 1970 > 50:
            ...
            assert any(
                r["condition"] == "BroadcastStillProtectedArticle3"
                for r in status["red"]
            )
    ...
    def test_broadcast_with_copyright_work(self):
        ...
        assert any(
            r["condition"] == "BroadcastProtectionLapsedArticle3"
            for r in broadcast_status["green"]
        )
    ...
    def test_broadcast_with_film_fixation(self):
        ...
        assert any(
            ...
            for r in film_fixation_status["red"]
        )
